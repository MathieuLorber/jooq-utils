...
object DependenciesParser {
    ...
    fun jooqDialect(driver: DatabaseConfiguration.Driver) =
        when (driver) {
    ...
    fun getDependenciesSet(
        ...
        run {
            val map: Map<Table<*>, References> =
                dependenciesSet
                    .flatMap { dependencies ->
                        dependencies.tables.map { it to dependencies.references }
                    }
                    .toMap()
            map.keys.forEach { checkReferences(it, map) }
        }

        logger.debug { "Dependencies :" }
        dependenciesSet.forEach {
            logger.debug {
                "* Tables ${it.tables.map { it.name }} : ${it.references.tables.map { it.name }}"
            }
        }
        logger.debug { "Dependencies :" }
