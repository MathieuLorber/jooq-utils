        ...
        DatasourcePool.get(conf).connection.use {
            it.createStatement().use { statement ->
                ...
                classified.constraints.forEach { c ->
                    val sql =
                        "alter table ${c.table.schema?.let { "$it." } ?: ""}${c.table.name} drop constraint ${c.constraintName}"
                    logger.debug { "Execute \"$sql\"" }
                    statement.execute(sql)
                    sb?.appendLine(sql + ";")
                    sb?.appendLine()
    ...
    private fun dropTables(
        ...
        val dropTables =
            reverseDependencies
                ...
                .toSet()
        dropTables.forEach {
            logger.debug { "Drop table ${it.schema?.let { "$it." } ?: ""}${it.name} (if exists)" }
            val sql = "drop table if exists ${it.schema?.let { "$it." } ?: ""}${it.name}"
            statement.execute(sql)
            sb?.appendLine(sql + ";")
            sb?.appendLine()
        }
